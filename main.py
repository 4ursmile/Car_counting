...
st.set_page_config(page_title="Vehicle Couting", page_icon="🚗", layout="wide")
st.title("Vehicle Search")

st.markdown("This is a demo for vehicle couting")
video = None
take_class = None
video_folder = "./Videos"
class CancleState:
...
with st.form("InputForm"):
    ...
    conf_val = conf.slider("Confidence threshold", min_value=0.0, max_value=1.0, value=0.2, step=0.01)
    iou_val =  iou.slider("IoU threshold", min_value=0.0, max_value=1.0, value=0.4, step=0.01)
    st.subheader("Image resize")
    # input width and height on the same row
    widthc, heightc = st.columns(2)
    width = widthc.number_input("Width", min_value=0, max_value=10000, value=1280, step=1)
    height = heightc.number_input("Height", min_value=0, max_value=10000, value=720, step=1)

    st.subheader("Counting Parameters")
    a, d, l = st.columns(3)
    acceptable = a.number_input("Acceptable", min_value=0, max_value=1000, value=20, step=1)
    ...
    fc, ds = st.columns(2)
    targer_counter = fc.number_input("Skip n-th frame (ignore if framecut == false)", min_value=0, max_value=1000, value=1, step=1)
    dis_scalar = ds.number_input("Distance scalar", min_value=0, max_value=100, value=1, step=1)
    ...
    cap_w = cap_w.number_input("CWidth", min_value=0, max_value=10000, value=1280, step=1)
    cap_h = cap_h.number_input("CHeight", min_value=0, max_value=10000, value=720, step=1)
        ...
        while True:
            ...
            if ret:
                ...
                if show_cap:
                    ...
                    draw_with_alpha(roi, cv2.line, (0, line-acceptable), (cap_w, line-acceptable), (214, 214, 214), 2, 0.8)
                
                if len(results) > 0:
                    presults = tracker.update( results, classes=classes, liney = line, acceptable = acceptable, delta = base_dis*dis_scalar)
                    for i, package in enumerate(presults):
                        ux, uy, lx, ly, id, direction, clsid = package 
                        cv2.rectangle(roi, (ux, uy), (lx, ly), color_ref[clsid], 2)
                        cv2.putText(roi, f'id: {id} class: {class_ref[classes[i]]} conf: {confs[i]:0.2f}' , (ux, uy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_ref[clsid], 2)
                cv2.putText(frame, f"Vehicle Count: {str(tracker.count)}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                cv2.putText(frame, f"Active Vehicle: {str(len(results))}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
